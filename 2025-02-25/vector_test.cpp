...
TEST_CASE("Vector at() function", "[vector]")
{
    ...
    SECTION("Edge cases")
    {
        REQUIRE(at(v, -1) == ERROR_1);
        REQUIRE(at(v, 0) == ERROR_1); // Empty vector
        pushBack(v, 10);
        REQUIRE(at(v, 1) == ERROR_1); // Out of bounds
    }
}
...
TEST_CASE("Vector pop() function", "[vector]")
{
    ...
    SECTION("Edge cases")
    {
        REQUIRE(pop(v) == ERROR_1); // Empty vector
    }
}
...
TEST_CASE("Vector popFront() function", "[vector]")
{
    ...
    SECTION("Edge cases")
    {
        REQUIRE(popFront(v) == ERROR_1); // Empty vector
    }
}
...
TEST_CASE("Vector insert() function", "[vector]")
{
    ...
    SECTION("Edge cases")
    {
        REQUIRE(insert(v, -1, 10) == ERROR_1); // Negative index
        REQUIRE(insert(v, 1, 10) == ERROR_1);  // Out of bounds
        pushBack(v, 10);
        REQUIRE(insert(v, 2, 20) == ERROR_1); // Gap insertion
    }
}
...
TEST_CASE("Vector remove() function", "[vector]")
{
    ...
    SECTION("Edge cases")
    {
        REQUIRE(remove(v, 0) == ERROR_1); // Empty vector
        pushBack(v, 10);
        REQUIRE(remove(v, -1) == ERROR_1); // Negative index
        REQUIRE(remove(v, 1) == ERROR_1);  // Out of bounds
    }
}
